...
To add the AIRTOOLS binary package repository you must add a new entry
to the package managment sources list, e.g. by creating a file
`/etc/apt/sources.list.d/airtools.list` containing a line:

    deb [trusted=yes] http://fg-kometen.vdsastro.de/airtools/debian YOUR_DISTRIBUTION main

where `YOUR_DISTRIBUTION` has to be replaced by the codename of your
distribution (e.g. focal).
...
The AIRTOOLS software is updated every couple of months. An update of
your installation is issued by the following two commands:
